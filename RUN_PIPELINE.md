# Run the full pipeline on a new machine

This is the mentor handoff guide. Run the stages in this order: environment →
judge → full probe → three corpora → one-step GRPO smoke test → full GRPO runs.
The trained probe is a prerequisite for the probed-order corpus; a fresh clone
does not contain its weights. All commands below start from the repository root.
...
then `cd` to its root. The source distribution must include untracked new setup
files if the author has not committed them yet. The caches are deliberately ignored
by Git and are downloaded/created on the mentor's machine.
